...
//
// To create a new place for tracking statistics for a single model version, do:
//
//   ModelBatchStats& model_batch_stats =
//       stats.AddModelBatchStats(/* model_name= */ "m", /* op_name= */ "o");
//
// Then, to register a cost sample for a particular batch size, do:
//
//   model_batch_stats.batch_size(4).tpu_cost.Register(cost);
//
// To get the mean cost later, do:
//
//   std::optional<absl::Duration> cost =
//       model_batch_stats.batch_size(4).tpu_cost.mean();
//
...
class CostTracker {
 public:
  CostTracker() = default;
  CostTracker(CostTracker&&) = delete;
  CostTracker(const CostTracker&) = delete;
  void operator=(CostTracker&&) = delete;
  void operator=(const CostTracker&) = delete;
...
class BatchSizeStats {
 public:
  BatchSizeStats() = default;
  BatchSizeStats(BatchSizeStats&&) = delete;
  BatchSizeStats(const BatchSizeStats&) = delete;
  void operator=(BatchSizeStats&&) = delete;
  void operator=(const BatchSizeStats&) = delete;
...
//
// Here, "model" means a specific version of a model. In rare cases, when a
// model version has multiple BatchFunction operation, we also treat each such
// operation as a separate model in this context.
//
// Thread-safe.
class ModelBatchStats {
 public:
  explicit ModelBatchStats(const std::string& model_name,
                           const std::string& op_name)
      : model_name_(model_name), op_name_(op_name) {}

  ModelBatchStats(ModelBatchStats&&) = delete;
  ModelBatchStats(const ModelBatchStats&) = delete;
  void operator=(ModelBatchStats&&) = delete;
  void operator=(const ModelBatchStats&) = delete;
...
class BatchStats {
 public:
  BatchStats() = default;
  BatchStats(BatchStats&&) = delete;
  BatchStats(const BatchStats&) = delete;
  void operator=(BatchStats&&) = delete;
  void operator=(const BatchStats&) = delete;

  // Creates, registers, and returns a reference to a new ModelBatchStats
  // instance.
  //
  // When called multiple times with the same arguments, creates multiple copies
  // of ModelBatchStats. Therefore, the caller is responsible for ensuring
  // this method is only called once for a logical model (see the documentation
  // for ModelBatchStats for what a local model is).
  //
  // The returned reference persist for as long as 'this' is alive.
  ModelBatchStats& AddModelBatchStats(const std::string& model_name,
                                      const std::string& op_name) {
    mutex_lock l(mu_);
    return model_batch_stats_.emplace_back(model_name, op_name);
  }
 ...
 private:
  ...
  //
  // The mutex only protects adding/scanning elements in the deque. Access to
  // elements themselves (after they were created) is not protected here. No
  // element deletion is possible because we return references to items in this
  // deque and don't track their lifetime. We are using std::deque so that
  // elements, once created, are fixed in memory.
  std::deque<ModelBatchStats> model_batch_stats_ TF_GUARDED_BY(mu_);
};
