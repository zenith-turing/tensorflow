...
TEST(BatchStatsTest, BasicOperation) {
  ...
  model_batch_stats.batch_size(1).tpu_cost().Register(absl::Hours(5));
  ASSERT_EQ(model_batch_stats.batch_size(1).tpu_cost().mean(), absl::Hours(5));
}

TEST(BatchStatsTest, EachNewAddModelBatchStatsCreatesANewInstance) {
  BatchStats stats;
  ASSERT_NE(
      &stats.AddModelBatchStats(/* model_name= */ "m", /* op_name= */ "o"),
      &stats.AddModelBatchStats(/* model_name= */ "m", /* op_name= */ "o"));
...
TEST(BatchStatsTest, BatchSizeStatsAreUniqueForEachBatchSize) {
  ...
  ModelBatchStats& model_batch_stats =
      stats.AddModelBatchStats(/* model_name= */ "m", /* op_name= */ "o");
  ASSERT_NE(&model_batch_stats.batch_size(1), &model_batch_stats.batch_size(2));
}
