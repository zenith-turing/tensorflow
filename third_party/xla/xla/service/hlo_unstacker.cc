...
Shape MakeUnstackedShapeFromStacked(const Shape& stacked_shape) {
  int64_t num_layers = stacked_shape.dimensions(0);
  Shape slice_shape = ShapeUtil::DeleteDimension(0, stacked_shape);
  return MakeUnstackedShapeFromSlice(slice_shape, num_layers);
}
      ...
      if (Match(instr->fused_instructions_computation()->root_instruction(),
        ...
        if (bitcast_operand == fused_instr) {
          ...
          pattern_info.unstacked_shape =
              MakeUnstackedShapeFromStacked(instr->operand(0)->shape());
