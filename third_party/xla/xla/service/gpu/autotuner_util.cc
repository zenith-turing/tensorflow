...
absl::StatusOr<std::string> GetCacheFilePath(
    absl::string_view autotune_cache_dir, const AutotuneCacheKey& key) {
  if (autotune_cache_dir.empty()) {
  ...
  return tsl::io::JoinPath(autotune_cache_dir,
...
absl::Status AddResultToFileBasedCacheIfEnabled(
    std::string_view autotune_cache_dir, const AutotuneCacheKey& key,
    AutotuneResult result) {
  if (autotune_cache_dir.empty()) {
  ...
  TF_ASSIGN_OR_RETURN(const std::string file_path,
                      GetCacheFilePath(autotune_cache_dir, key));
...
absl::StatusOr<ResultAndInserted> AddResultToCaches(const AutotuneCacheKey& key,
  ...
  if (result_and_inserted.inserted) {
    ...
    TF_RETURN_IF_ERROR(AddResultToFileBasedCacheIfEnabled(
        autotune_cache_dir, key, result_and_inserted.result));
  }
...
/*static*/ absl::Status AutotunerUtil::LoadAutotuneResults(
  ...
  for (const AutotuneResults::Entry& result : results.results()) {
    const AutotuneCacheKey key =
        AutotuneCacheKey(result.device(), result.hlo());
    auto [it, inserted] = autotune_cache.emplace(key, result.result());
    if (!inserted) {
      return absl::InternalError(absl::StrCat(
          "Duplicate autotuning result for ", it->first.ToString()));
    }
    TF_RETURN_IF_ERROR(AddResultToFileBasedCacheIfEnabled(autotune_cache_dir,
                                                          key, it->second));
  }
  return absl::OkStatus();
}
...
static absl::StatusOr<std::optional<AutotuneResult>> TryFindInCache(
  ...
  TF_ASSIGN_OR_RETURN(opt_result, TryToFindInFileBasedCacheIfEnabled(key));
  if (opt_result.has_value()) {
...
/*static*/ absl::StatusOr<bool> AutotunerUtil::AddResult(
  ...
  TF_ASSIGN_OR_RETURN(ResultAndInserted result_and_inserted,
                      AddResultToCaches(key, std::move(result)));
...
/*static*/ void AutotunerUtil::SetPerFusionAutotuneCacheDir(
  ...
  absl::MutexLock lock(&autotune_cache_mu);
  autotune_cache_dir = new_autotune_cache_dir;
}
