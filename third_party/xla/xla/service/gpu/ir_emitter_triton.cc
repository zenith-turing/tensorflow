 ...
 public:
  ...
  mlir::FloatType GetDotAccumulatorType() {
    ...
    if (algorithm == PrecisionConfig::ALG_UNSET) {
      ...
      // The code below assumes that lhs and rhs has the same type. However it's
      // not always the case with fp8 matmuls, e.g. e4m3×e5m2 is supported at
      // the hardware level. NVidia GPU currently only supports f32 accumulator
      // for such matmuls.
...
absl::Status EmitMatMul(mlir::OpBuilder builder,
  ...
  auto body_builder = [&](mlir::OpBuilder&, mlir::Location, Value ki,
    ...
    if (Is6xBfloat16MatMul(dot_instr, b, dot_input_lhs, dot_input_rhs,
      ...
      int max_num_imprecise_acc = IsFp8Matmul(dot_instr) ? (1 << 30) : 0;
      accumulator_next =
